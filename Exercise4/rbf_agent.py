...
class Agent(object):
    ...
    def single_update(self, state, action, next_state, reward, done):
        ...
        next_qs = np.array(np.amax(next_qs, axis=0))
        
        # Calculate the updated target Q- values
        # Task 1: TODO: Calculate target based on rewards and next_qs
        #We use the TD(0), to measure the target.
        target=np.array([reward+self.gamma*next_qs*(1.0-done)])
    ...
    def update_estimator(self):
        ...
        for transition in samples:
            st, ac, nest, rew, dons = transition
            states.append(st)
            action.append(ac)
            rewards.append(rew)
            next_states.append(nest)
            dones.append(dons)
        ...
        next_qs=[]
        for j in range (len(featurized_next_states)):
            next_qs.append(np.amax(np.array([q.predict(np.array(self.featurize(next_states[j])))[0] for q in self.q_functions]), axis=0))
        next_qs=np.array(next_qs)


        # Calculate the updated target values
        # Task 2: TODO: Calculate target based on rewards and next_qs
        targets = rewards+next_qs*self.gamma*(1-dones)
