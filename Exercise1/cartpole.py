...
def new_reward(state):
    ...
    """rew = 2.0
    pos0 = 0.0
    actualpos = abs(state[0])
    return (rew-2*(actualpos-pos0))"""
    
    #Exercise 2-Position  
    """rew=4/(20*abs(args.position-state[0])**2+1)    
    return(rew)"""
    #Exercise 3-Velocity
    
    if state[0]<1.75 and state[0]>-1.75:
        rew=1+state[1]**2
    if state[0]>1.75:
        if state[1]<0:
            rew=3+state[1]**2
...
def main(args):
    ...
    #print("Position Desired:", args.position)
